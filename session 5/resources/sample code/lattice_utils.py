...
def check_vecs(V1,V2):
    """
    Simple utility to check that V1 and V2 are of the same dimension 
    """
...
def recip_lattice(latt):
    ...
    c_vect = np.array([latt.c*np.cos(latt.aa), latt.c*np.cos(latt.bb),  np.sqrt(latt.c**2*(1-(np.cos(latt.aa))**2 + (np.cos(latt.bb))**2))])
